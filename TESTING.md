...
Every pull request and push to `main` runs these jobs (`.github/workflows/ci.yml`); on a documentation-only pull request the backend matrix and the frontend build are skipped, as described under "Documentation-only changes" below:
...
One consequence of the fail-closed sentinel: while the full suite is running, or before the label is added, the gate reports red, because "the full suite has not yet passed" is a deliberate failure rather than a softer pending state. The check turns green once the integrated full suite passes; branch protection evaluates the most recent result for the check, so an in-progress red is superseded. Day to day: iterate without the label (the fast per-PR gate only), then add `full-gate` when the change is reviewed and ready to merge.

### Documentation-only changes

A pull request that touches only documentation (every changed file is Markdown) needs neither the backend test matrix, the frontend build, nor the full-tier merge gate: there is no code to test and no frontend to build. The change-scope detection job classifies it, and both the per-push gate (`ci.yml`) and the merge gate (`full-gate.yml`) skip their heavy jobs for it.

Skipping a *required* check is the hazard: branch protection treats a never-reported required check as pending (deadlocking the merge) and a skipped one as passing (fail-open). Both gates avoid this with the same always-running sentinel pattern the pre-merge full gate uses. The `CI gate` (standing in for the backend and frontend contexts) and the `Pre-merge full gate` each always report, and each is fail-closed: a documentation-only change passes on that verdict alone, but a detection that did not run cleanly, or a code change whose jobs did not pass, fails the gate. So a documentation-only pull request goes green in seconds without a label, while a code change still runs and must pass everything. The classification is deliberately conservative: anything other than Markdown (source, configuration, the workflow files themselves, an asset) counts as code, so the safe direction (run the suite) is the default whenever there is any doubt.
