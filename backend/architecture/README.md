...
The consumer discipline is subscribe-then-hydrate: attach the Tauri listener first, then run the initial read, so a change landing between the first read and the listener attach is re-announced rather than lost. The ordering is the consumer's responsibility, not a store guarantee: `frontend/src/lib/stores/trackingStore.ts` and `scanStore.ts` expose `subscribe` and `hydrate` as independent calls and contribute the shared read mechanics (a relayed frame is a pure trigger; `hydrate` coalesces overlapping reads, with a frame arriving mid-read queueing exactly one follow-up, and a failed read keeps the last good snapshot rather than blanking). The dashboard and quests routes are the canonical consumers: each initiates the subscribe first (capturing the unlisten when the Tauri listener attaches), then runs the initial hydrate independently of that `listen()` promise, so the first read still happens in environments where the promise never resolves (the residual is a frame arriving in the window before the listener attaches, far smaller than reading before subscribing at all); the overlay and scan-overlay windows attach their own local listeners to the same topics with their own local refresh functions.
