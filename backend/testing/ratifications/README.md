...
`<slug>.md` and added or modified in the same pull-request range. The report
ends in a fenced verdict block the guard parses:
...
The gate passes only on `VERDICT: ratification-sound`, and only when the
`goldens:` field names every changed set. See the "Independent ratification"
