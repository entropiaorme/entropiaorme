"""DSL build script for the consistency_scan_isolation_midpoint scenario.

Regenerate the scenario's two segment files via::

    python -m backend.tests.e2e.corpus.scripted.consistency_scan_isolation_midpoint.build
...
the scan surface's projection must stay unchanged across the interval.
"""
...
def build() -> Path:
    ...
    pre = Scenario(name="consistency_scan_isolation_midpoint_pre")
    pre.at("2026-05-19 12:00:00")
    pre.combat.damage_dealt(15.0)

    _write_segment(pre.lines(), target_dir / "chat_replay.log")

    post = Scenario(name="consistency_scan_isolation_midpoint_post")
    post.at("2026-05-19 12:00:10")
    post.combat.damage_dealt(22.0)
    post.tick()
    post.combat.critical_hit(35.0)
    post.tick()
    post.loot.received("Animal Hide", value_ped=4.20)
