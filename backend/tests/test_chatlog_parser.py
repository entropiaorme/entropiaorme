...
@pytest.mark.parametrize("line,expected_type,expected_data", [
    # ── Player attacks ────────────────────────────────────────────────────────
    (
...
def test_parse_event(line, expected_type, expected_data):
    ...
    for key, value in expected_data.items():
        assert event.data[key] == value, f"data[{key!r}]: got {event.data[key]!r}, expected {value!r}"
...
def test_all_event_types_covered():
    ...
    covered = {
        EventType.DAMAGE_DEALT, EventType.CRITICAL_HIT,
        EventType.TARGET_JAM, EventType.TARGET_DODGE, EventType.TARGET_EVADE,
        EventType.DAMAGE_RECEIVED, EventType.DEFLECT,
        EventType.PLAYER_EVADE, EventType.PLAYER_DODGE, EventType.PLAYER_JAM,
        EventType.MOB_MISS, EventType.SELF_HEAL,
...
def test_loot_quantity_extraction():
    ...
    line = "2026-03-24 14:31:00 [System] [] You received Shrapnel x (342) Value: 3.42 PED"
    event = parse_line(line)
    assert event is not None
...
def test_loot_no_quantity():
    ...
    line = "2026-03-24 14:31:00 [System] [] You received Animal Muscle Oil Value: 0.12 PED"
    event = parse_line(line)
    assert event is not None
    assert event.type == EventType.LOOT
