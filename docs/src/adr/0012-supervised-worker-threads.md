...
> **Transport update ([ADR-0013](0013-in-process-collapse.md)).** The supervision rule below is unchanged, and the port it anticipated has landed: the backend is now a single in-process Rust service, so the owned, joined-on-shutdown worker handle the decision pinned is the shipped idiom rather than a target. The Python construction described below remains as the cross-language testing oracle ([ADR-0005](0005-cross-language-equivalence-oracle.md)), not a shipped runtime.
