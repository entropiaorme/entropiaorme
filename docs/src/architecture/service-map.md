...
| `eo-wire` | The wire-format contracts: the typed domain-event union, the event-stream fan-out hub, the domain-event channel, and the canonicalising emitters that produce the frozen equivalence goldens. | (leaf) |
...
| `dev_routes` | The developer-mode-gated routes (the metrics snapshot and the crash-reporting toggle), native-only and off the equivalence surface; they answer `404` when developer mode is disabled. |

The substrate's request path nests two `axum` middleware layers around routing (`build_router` in `frontend/src-tauri/eo-http/src/lib.rs`): `cors_layer` is the outermost layer (a preflight short-circuits routing and the guards when the contract is configured), then the Host and origin guard (`api_guard`), then routing. The CORS contract those layers enforce lives in `frontend/src-tauri/eo-http/src/cors.rs`. This nesting reproduces the layering the earlier Python backend served behind. Strong ETags are computed over the response body with SHA-256 in the hydration handlers. An unmatched path is the framework's own `404`, and an unhandled method on a registered path is a `405`.

### `eo-services`: the domain services

Defined by `frontend/src-tauri/eo-services/src/lib.rs`. This crate carries the service layer behind the HTTP surface, built up service by service over the course of the port. Its modules group as follows:
...
The settings `PATCH /api/settings` and `POST /api/settings/reset` writes are served natively: the handler validates the patch, writes through the configuration service, and signals the live producers (restarting the chat-log watcher on a path change, toggling the hotbar hooks, and reloading the tracker configuration) so a settings change reconciles without a restart. Domain events no longer travel over an HTTP route; they reach the frontend over the Tauri event bridge described in the [System overview](overview.md). Beyond the groups above, the router also registers the developer-mode-gated dev-tools routes (`GET /api/dev/metrics`, `GET`/`POST /api/dev/crash-reporting`); these are native-only, gated on developer mode, and answer `404` when it is disabled, so they sit off the equivalence-covered surface. Every unregistered path is the framework `404`, and an unported method on a registered path is a `405`.
