...
#[must_use = "dropping the telemetry guard tears down/flushes the subscriber's appender"]
pub struct TelemetryGuard {
    // Populated once the non-blocking rolling appender lands; until then the
    // subscriber writes synchronously to stderr and there is nothing to flush.
    _appender: Option<()>,
}
...
pub fn init() -> TelemetryGuard {
    // Default to INFO, but quiet the database driver's own per-query INFO
    // logging (one line per query) to WARN so the console and the rolling
    // log file are not flooded; the metrics layer still observes every query
    // for the latency histogram through its own target filter, independent of
    // this one. Override wholesale via ENTROPIAORME_LOG (standard EnvFilter
    // syntax, e.g. `debug` or `eo::ocr=trace,info`).
    let env_filter = EnvFilter::try_from_env("ENTROPIAORME_LOG")
